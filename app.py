...
with col1:
    for player in range(1, half + 1):
        name = st.text_input(f":orange[Player {player}:]", placeholder=None, key=f'{player}')
        my_dict.update({player: name.strip()})
        
with col2:
    for player in range(half + 1, ppl + 1):
        name = st.text_input(f":orange[Player {player}:]", placeholder=None, key=f'{player}')
        my_dict.update({player: name.strip()})
...
with stack1:
    st.write("_Please fill in all names_")
with stack2:
    if button == True:
        st.button("Randomize Order", use_container_width=True)
with stack3:
    if st.button("Clear"):
